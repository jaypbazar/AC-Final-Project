...
def sha1_hash():
    if request.method == 'POST':
        ...
        if input_type == 'file' and 'file' in request.files:
            ...
            # Read file content as text
            content = file.read().decode('utf-8')
            hash_result = SHA1Hash.generate_hash(content)
        else:
            text = request.form.get('text')
...
def verify_sha1():
    try:
        ...
        if input_type == 'file' and 'file' in request.files:
            ...
            # Read file content as text
            content = file.read().decode('utf-8')
            is_match = SHA1Hash.verify_hash(content, hash_to_verify)
        else:
            text = request.form.get('text')
            if not text or not hash_to_verify:
                return jsonify({'error': 'Missing required parameters'}), 400
...
def sha256_hash():
    if request.method == 'POST':
        ...
        if input_type == 'file' and 'file' in request.files:
            ...
            # Read file content as text
            content = file.read().decode('utf-8')
            hash_result = SHA256Hash.generate_hash(content)
        else:
            text = request.form.get('text')
...
def verify_sha256():
    try:
        ...
        if input_type == 'file' and 'file' in request.files:
            ...
            # Read file content as text
            content = file.read().decode('utf-8')
            is_match = SHA256Hash.verify_hash(content, hash_to_verify)
        else:
            text = request.form.get('text')
            if not text or not hash_to_verify:
                return jsonify({'error': 'Missing required parameters'}), 400
